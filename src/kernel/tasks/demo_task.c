...
void heartbeat_task(void)
{
    ...
    log_append_current_task("heartbeat: started ", counter);
    ...
    while (1)
    {
        ...
        if (counter == 1)
        {
            log_append_current_task("heartbeat: alive count=", counter);
        }
...
void worker_fast_task(void)
{
    ...
    while (1)
    {
        for (volatile int i = 0; i < 10000; i++)
        {
            ...
            if (counter % 50 == 0)
            {
                log_append_current_task("worker_fast: iterations=", counter);
            }
...
void worker_slow_task(void)
{
    ...
    while (1)
    {
        for (volatile int i = 0; i < 100000; i++)
        {
            ...
            if (counter % 10 == 0)
            {
                log_append_current_task("worker_slow: iterations=", counter);
            }
